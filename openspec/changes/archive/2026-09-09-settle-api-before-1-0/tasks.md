...
- [x] 1.1 Delete `Patched<T>` from `kdiff-runtime/.../Patch.kt` and change every helper — including
...
- [x] 1.2 Regenerate the sample with `./gradlew :kdiff-sample:kspKotlin --rerun-tasks` and verify every
  generated file is textually identical to before, proving the processor needed no change.

## 2. `Diff`

- [x] 2.1 Remove `data` from `Diff` in `Diff.kt`, adding explicit `equals`, `hashCode` and a `toString`
...
- [x] 2.2 Add a compile-failure check — a kctfork snippet in `kdiff-processor`'s test source set, which
...
- [x] 3.1 Mark `Differ<T>` and `Patcher<T>` as `fun interface` (design D4). Verify `DslSpec` gains a
...
- [x] 3.2 Add `Segment.Key.MAP_ENTRY` in `FieldPath.kt` and use it in `compareMap` and `patchMap`
...
- [x] 3.3 Run `./gradlew :kdiff-runtime:updateKotlinAbi` and verify the diff is exactly: `Patched` and
...
- [x] 4.1 Run `./gradlew :kdiff-sample:test :kdiff-tutorial:test` and verify both pass; `MoneyDiffer`
  in the sample reads `.value` and compiles unchanged. Fix any test that asserted on `Diff`'s old
  data-class `toString`.

## 5. Documentation

- [x] 5.1 Write `docs/api-stability.md` with the seven sections of design D6, link it from `README.md`
...
- [x] 5.2 Update `docs/patching.md` (re-cite the `PatchResult` declaration; no `Patched`),
...
- [x] 5.3 Add the `CHANGELOG.md` entries under `[Unreleased]`: **BREAKING** *Changed* for `Patched`
  and for `Diff.copy`, each with its one-line migration; *Added* for the functional interfaces,
  `MAP_ENTRY` and the stability page.

## 6. Done

- [x] 6.1 Run `./gradlew check` and verify it passes, including ABI validation against the dump updated
  in 3.3.
