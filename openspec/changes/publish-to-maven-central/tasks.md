...
- [ ] 1.1 Answer design D2's open question — whether a `kdiff` GitHub organisation is the maintainer's —
...
- [ ] 2.1 Read the publishing plugin's current release and record its version in
...
- [ ] 2.2 Configure the javadoc jar to package each module's Dokka HTML output, keeping `dokkaGenerate`
  out of `check` (design D3). Verify `./gradlew check` produces no documentation and
  `./gradlew publishToMavenLocal` does.
- [ ] 2.3 Delete the `GitHubPackages` repository block and its credential properties (design D5).
  Verify no reference to `maven.pkg.github.com` or `gpr.` remains outside `CHANGELOG.md`.
- [ ] 2.4 Run `./gradlew publishToMavenLocal` and inspect `~/.m2/repository/<group path>/` for each
...
- [ ] 2.5 Update `DocumentationSamplesSpec`'s coordinate regex to the verified group id (design D8).
...
- [ ] 3.1 Replace the `publish` step in `.github/workflows/publish.yml` with the plugin's
...
- [ ] 3.2 Outside the repository, once: verify the group id namespace on the Central Portal, generate
...
- [ ] 4.1 Rewrite `README.md` *Install*: remove the warning box and the repository block, show the three
...
- [ ] 4.2 Add a *Releasing* section to `CONTRIBUTING.md` covering the namespace verification, the
...
- [ ] 4.3 Add the `CHANGELOG.md` entry under `[Unreleased]`: published to Maven Central with sources and
  ...
  to upgrade.
- [ ] 4.4 Update `docs/faq.md`'s install pointer to drop the token mention. Verify the page's
  `<!-- from: -->` blocks are unaffected.

## 5. Done

- [ ] 5.1 Run `./gradlew check` and verify it passes with no ABI dump change, since no published source
  moved.
