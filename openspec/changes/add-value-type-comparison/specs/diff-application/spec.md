...
value class, a type declared `@DiffAsValue`, or a property declared `@DiffAsValue` — SHALL set the
property to the change's new value, whatever that value's type. A `@DiffAsValue` property whose type is
`@Diffable` or a collection SHALL be set wholesale, since the change carries the whole new value and
nothing was reported beneath it.
