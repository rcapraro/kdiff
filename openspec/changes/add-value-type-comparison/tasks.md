...
- [ ] 1.1 Add `DiffAsValue` to `kdiff-annotations/.../Properties.kt` with `CLASS` and `PROPERTY`
...
- [ ] 2.1 Extend `VALUE_TYPES` in `Resolution.kt` with the qualified names from design D2, and make
...
- [ ] 2.2 Confirm whether `kotlin.time.Instant` and `kotlin.uuid.Uuid` are stable in the pinned
  ...
  proved.
- [ ] 2.3 Add a kctfork scenario that `java.util.Date` is still rejected, so the set is shown to be a
  list and not a package.

## 3. Processor — `@DiffAsValue`

- [ ] 3.1 In `resolve()`, return `Comparison.ByValue` for a property carrying `@DiffAsValue` after the
...
- [ ] 3.2 Report the five design D5 diagnostics — the class-level conflict with `@Diffable`, the
  ...
  location.
- [ ] 3.3 Amend the two "cannot compare" messages per design D6. Verify `DiagnosticSpec`'s existing
  substring assertions still pass and add one asserting `@DiffAsValue` appears in each.
- [ ] 3.4 Run `./gradlew :kdiff-processor:test` and verify every existing spec passes unchanged.

## 4. Sample and tutorial

- [ ] 4.1 Add `val discount: BigDecimal`, `val placedAt: Instant`, a `@JvmInline value class Sku`
...
- [ ] 4.2 Extend `OrderDiffSpec` and `RoundTripSpec` with the new properties, including the
...
- [ ] 4.3 Add a `RecipesSpec` test for the how-to recipe *Compare a type as a single value*, showing a
  class-level and a property-level `@DiffAsValue`. Verify it carries the `from:` marker the docs cite.
- [ ] 4.4 Revise the identifier comment in `kdiff-tutorial/.../domain/Person.kt` and the matching
...
- [ ] 5.1 Update `docs/errors.md` with the five D5 messages and the two D6 amendments, verbatim, and the
  tier-1 count. Verify each quoted string matches the emitting `KSPLogger.error` call by search.
- [ ] 5.2 Update `docs/annotations.md` (the `@DiffAsValue` section, the *What counts as a value* list
...
- [ ] 5.3 Add the `docs/how-to.md` recipe citing the 4.3 test.
- [ ] 5.4 Add the `CHANGELOG.md` *Added* entry under `[Unreleased]` listing the three routes and the
  accepted types, written for someone deciding whether to upgrade.

## 6. Done

- [ ] 6.1 Run `./gradlew check` and verify it passes, including ABI validation against the annotations
